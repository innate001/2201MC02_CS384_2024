...
def process_file(input_path):
    ...
    # Create the scaled marks column for each student based on their grade
    def scale_marks(grade, marks):
        grade_ranges = {
            'AA': (91, 100),
            'AB': (81, 90),
            'BB': (71, 80),
        ...
        a, b = grade_ranges.get(grade, (0, 100))  # default to 0-100 for any unrecognized grade
        min_marks = marks.min()
        max_marks = marks.max()
        scaled_marks = (b - a) * (marks - min_marks) / (max_marks - min_marks) + a
        return scaled_marks

    # Apply scaling to each grade group
    df_sorted_by_marks['Scaled Marks'] = df_sorted_by_marks.groupby('Grade')['Grand Total/100'].transform(lambda x: scale_marks(x.name, x))

    # Output the new DataFrame with scaled marks
    df_scaled = df_sorted_by_marks[['Roll', 'Grade', 'Grand Total/100', 'Scaled Marks']]
    ...
    with pd.ExcelWriter(output_excel_file) as writer:
        ...
        df_scaled.to_excel(writer, sheet_name='Scaled Marks by Grade', index=False)
